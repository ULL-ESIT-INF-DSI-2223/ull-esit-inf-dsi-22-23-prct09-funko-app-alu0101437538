...
export class Funko {
    ...
    constructor(
        ...
        if (/\.|-/.test(_id.toString()) === true || _id === undefined ) {
        ...
        if (_franquicia === undefined ) {
            throw new Error("El funko debe de tener franquicia")
        }
        if (_exclusivo === undefined ) {
            throw new Error("El funko debe de dejar clara su condición de exclusividad")
        }
    }
    ...
    print(): string {
        ...
        if (this._caracteristicasEsp !== undefined ) {
            str += `\nCaracterísticas Especiales = ${this._caracteristicasEsp}`
            console.log(chalk.blue(`Características Especiales =`) + ` ${this._caracteristicasEsp}`)
        }
        str += `\nValor = ${this._valor}`
        if (this._valor < 15) {
