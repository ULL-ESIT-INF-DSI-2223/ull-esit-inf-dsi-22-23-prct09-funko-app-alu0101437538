...
export class Gestor {
    ...
    public static addFunko(nombre: string, funko: Funko): void {
        if (!this.checkUserCreated(nombre)) {
            throw new Error("No existe un usuario con ese nombre. Por favor, creerlo con --addUser.")
        } 
    ...
    public static readFunko(nombre: string, id: number): Funko {
        if (this.checkFunkoCreated(nombre, id)) {
            const json = JSON.parse(fs.readFileSync("./dataBase/" + nombre + "/" + id + ".json").toString())
            const funko = new Funko(json._id, json._nombre, json._descripcion, json._tipo, json._genero, 
                json._franquicia, json._numero, json._exclusivo, json._caracteristicasEsp, json._valor)
            funko.print()
            return funko
        } else {
            throw new Error("No existe funko con ese id.")
