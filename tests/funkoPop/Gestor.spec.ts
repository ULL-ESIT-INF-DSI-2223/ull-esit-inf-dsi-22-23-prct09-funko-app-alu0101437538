...
describe("Test de métodos de Gestor + errores", () => {
    ...
    it("No se puede crear un funko a un usuario que no existe", () => {
        const funko1 = new Funko(1,"Goku", "El legendario supersaiyan", Tipos.Pop , Generos.Animacion,"Dragon Ball", 7, false, undefined, 13.99)
        expect(() => Gestor.addFunko("FRGBWRNOGHAPROUGHFDSIOUBGLADFSIVBGAPOVDNAOUIB", funko1)).to.be.throw();
    });
    ...
    it("Se puede editar un funko + ReadFunko", () => {
        const funkoEdited = new Funko(1,"Goku", "El terrestre", Tipos.Pop , Generos.Anime,"Dragon Ball", 77, false, undefined, 33.99)
        Gestor.modFunko("UsiarioDePrueba12345", funkoEdited)
        expect(Gestor.readFunko("UsiarioDePrueba12345", funkoEdited.id)).to.be.eql(funkoEdited);
    });
    it("No se puede editar un funko que no existe", () => {
        const funko1 = new Funko(5,"Goku", "El legendario supersaiyan", Tipos.Pop , Generos.Animacion,"Dragon Ball", 7, false, undefined, 13.99)
        expect(() => Gestor.modFunko("UsiarioDePrueba12345", funko1)).to.be.throw();
    });
    it("No se puede crear un funko a un usuario que no existe", () => {
        const funko1 = new Funko(1,"Goku", "El legendario supersaiyan", Tipos.Pop , Generos.Animacion,"Dragon Ball", 7, false, undefined, 13.99)
        expect(() => Gestor.modFunko("FRGBWRNOGHAPROUGHFDSIOUBGLADFSIVBGAPOVDNAOUIB", funko1)).to.be.throw();
    });
    it("No se puede leer un funko que no existe", () => {
        const funko1 = new Funko(1,"Goku", "El legendario supersaiyan", Tipos.Pop , Generos.Animacion,"Dragon Ball", 7, false, undefined, 13.99)
        expect(() => Gestor.readFunko("FRGBWRNOGHAPROUGHFDSIOUBGLADFSIVBGAPOVDNAOUIB", funko1.id)).to.be.throw();
    });
    it("Se puede imprimir la lista de funkos", () => {
        const funkoEdited = new Funko(13,"Goku", "El terrestre", Tipos.Pop , Generos.Anime,"Dragon Ball", 77, false, undefined, 33.99)
        Gestor.addFunko("UsiarioDePrueba12345", funkoEdited)
        expect(Gestor.funkoList("UsiarioDePrueba12345")).to.be.eql(["1.json", "13.json"]);
    });
