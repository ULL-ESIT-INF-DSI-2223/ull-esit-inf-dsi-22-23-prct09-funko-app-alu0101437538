...
import { FilterMapProdReduce } from "../../src/ejercicio-pe1/FilterMapProdReduce.js";

describe("Test de FilterMapProdReduce", () => {
  it("Se instancia la clase correctamente", () => {
    expect(
      new FilterMapProdReduce(
        [1, 3, 4, 6],
        (i) => i < 5,
        (i) => i * 2
      )
    ).to.be.instanceOf(FilterMapProdReduce);
  });
  it("Existen getters y setters para el atributo de la colección de números", () => {
    const algoritmo = new FilterMapProdReduce(
      [1, 3, 4, 6],
      (i) => i < 5,
      (i) => i * 2
    );
